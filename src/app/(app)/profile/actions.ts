...
export async function updateJobPreferences(
  ...
  // Save to database (no validation required - user can clear preferences)
...
export async function updateCareerAspirations(
  ...
  // Save to database (empty summary is allowed - user can clear it)
  const { error: updateError } = await supabase
    .from("users")
    .update({
